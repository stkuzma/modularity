 ...
 */
final readonly class RevokeRole implements Processor
{
    ...
    public function process(mixed $input): null
    {
        ...
        $this->roles->revokeFromUser($role, $input->userId);

        $this->audit->record('access.role.revoked', 'user', $input->userId, [
            'role' => $role->name,
        ]);
